...
async function checkScore() {
    ...
    const scores = user.recommendationScores || {};
    // Note: In raw MongoDB findOne, a Map is just an Object
    const score1 = scores["1"];
    const isSolved = (user.uniqueSolvedIds || []).includes("1");

    console.log(`Problem 1 - Score: ${score1}, Solved: ${isSolved}`);
    console.log(`User uniqueSolvedIds: ${JSON.stringify(user.uniqueSolvedIds)}`);
